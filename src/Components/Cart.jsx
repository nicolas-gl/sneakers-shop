...
export default function Cart({closeCard, deleteItem, cartItems=[]}) {
  return (
    <div className={styles.cartOverlay}>
      <aside className={styles.cartPopup}>
        <h2>Корзина 
          <img className={styles.cartItemRemove} onClick={closeCard} src="/img/icons/del-close.svg" alt="Close Card" />
        </h2>
        {/* {cartItems.length === 0 
          ? <h1>пусто</h1> : <h1>не пусто</h1>} */}
        ...
        <div className={styles.emptyCard}>
          ...
          <button className={styles.cartBackward} onClick={closeCard}>Вернуться назад
        ...
        {/* <div className={styles.cartItems}>
          {cartItems.map(item => (
            <div key={item.id} id={item.id} className={styles.cartItem}>
              ...
              <img className={styles.cartItemRemove} onClick={() => deleteItem(item.id)} src="/img/icons/del-close.svg" alt="DeleteItem" />
            </div>
          ))}
        </div>
